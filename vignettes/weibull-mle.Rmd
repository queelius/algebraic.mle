...
l.wei <- function(theta) sum(dweibull(sim.df$lifetime,
                                      ...
                                      scale=theta[2],
                                      log=T))
score.wei <- function(theta) numDeriv::grad(l.wei,theta)
info.wei <- function(theta) -numDeriv::hessian(l.wei,theta)
```

An MLE is a point $(\hat\lambda,\hat k)'$ that is a maximum of `l.wei` over the
parameter's domain. We find the MLE with the following R code:
```{r}
mle.wei <- algebraic.mle::mle_newton_raphson(
    l=l.wei,
    max_iter=100,
    theta0=sim.theta,
    score=score.wei,
...
stddev <- sd(sim.df$lifetime)

print(mu)
print(stddev)

print(l.norm(c(mu,stddev)))

mle.norm <- algebraic.mle::mle_gradient_ascent(
    l.norm,
    max_iter=100,
...
mle.norm2 <- algebraic.mle::mle_newton_raphson(
    l.norm,
    theta0=algebraic.mle::point(mle.norm),
    max_iter=100,
    sup=function(x) x[2] > 0,
    debug=T)


print(l.norm(algebraic.mle::point(mle.norm)))
summary(mle.norm)

mle_normal(sim.df$lifetime)
```
...
```{r}
mle.theta <- algebraic.mle::point(mle.wei)
...
densities <- outer(shapes,scales,
                   \(x,y) mvtnorm::dmvnorm(cbind(x,y),mle.theta,vcov(mle.wei)))
