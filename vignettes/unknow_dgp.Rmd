...
```{r}
library(algebraic.mle)
score.wei <- weibull_score(sim.df$lifetime)
info.wei <- weibull_fim(sim.df$lifetime)
mle.wei.newton <- newton_raphson(
    par=start$par,
    fn=ll.wei,
    gr=score.wei,
    hess=function(x) -info.wei(x),
    control=list(
        fnscale=-1,
        eta=.1,
        trace=TRUE,
...
print(mle.wei.newton$par)
print(mle.wei.newton$value)
mle.wei.nr <- mle_numerical(mle.wei.newton)
summary(mle.wei.nr)
```

The `trace_info` field of the `mle` object contains the history of the
log-likelihood values as the algorithm progresses. Here is the code to plot
the log-likelihood values as the algorithm progresses:
```{r fig.align='center', fig.height=4, fig.width=6, echo=F}
loglikes <- -mle.wei.newton$trace_info[,"value"]
plot(loglikes, type="l", xlab="iteration", ylab="log-likelihood")
```
We see that it converges quite quickly to the MLE. (We slowed down the algorithm
by setting the learning rate `eta=.1` to make the plot more readable. Otherwise,
it only needed 2 iterations to converge.)

In the `algebraic.mle` package, we provide a precise and efficient local
search method, `mle_weibull`, for finding the MLE. It only needs a starting
point for the shape parameter $k$. Here is the code:
```{r}
k0 <- start$par[1]
cat("initial guess for k0 =", k0, "\n")
mle.wei <- mle_weibull(sim.df$lifetime, k0=k0)
```

The function, `mle_weibull`, returns an `mle` object that has an
