...
```{r, fig.align='center', echo=F, cache=T}
dgp.density <- density(
  rweibull(n=10000000,
           shape=sim.shape,
           scale=sim.scale) +
...
data) or the sample was small, then we need to make some assumptions about the
data.
...
```{r}
newton_raphson <- function(f, df, x0, tol=1e-4, max_iter=100L)
{
  fx <- f(x0)
  for (i in 1:max_iter)
  {
...
Here's the code:
```{r}
library(numDeriv) # for numerical derivatives/hessians
newton_raphson(
...
distribution in the `algebraic.mle` R package.

This function, `mle_weibull_shape_scale`, returns an `mle` object, which has an
...
In black, we have the true density. In green, we have the Weibull density.
In red, we have the normal density. From the plot, it's hard to tell which
distribution is a better fit to the DGP.

How do we choose between the Weibull and the normal distribution? We will
...
We have also shown how to fit a normal distribution to the same data.
...
test, and a lower AIC value. Thus, it may be the better fit, but we see that
the true DGP is visually different from both the Weibull and the normal
