...
## Bootstrap method
If we wish to *estimate* the bias when $F$ is not known (e.g., $\sigma^2$ is not
know), we may use the Bootstrap method.
...
$$
\widehat{\operatorname{bias}}(\hat\theta) = E_{\hat F}\bigl\{s(X^*)\bigr\} - t(\hat F)
$$
...
$$
\widehat{\operatorname{bias}}(\hat\theta) =
