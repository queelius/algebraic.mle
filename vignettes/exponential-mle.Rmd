...
$$
    \ell(\lambda) = \sum_{i=1}^n \log f(x_i;\lambda) = \sum_{i=1}^n
        \bigl\{ \log \lambda - \lambda x_i \bigr\}. 
$$
...
Since `mle_exp` returns an `mle` object, we can use various methods and
...
Next, we plot the respective densities of the non-asymptotic sampling distribution of
$\hat\theta$ and the theoretical asymptotic distribution.
```{r fig.align='center',fig.width=6,fig.height=4,echo=F,message=F,warning=F,cache=T}
B <- 500000
...
# density of data0
f.emp <- density(data0)
plot(f.emp,main='',xlab='',ylab='',xlim=c(1,6),ylim=c(0,.8), col='blue')
curve(dnorm(x,mean=rate,sd=rate/sqrt(n)),add=T, col='red',lwd=2)
# legend
legend('topright',legend=c('true','asymptotic'),col=c('blue','red'),lwd=2)
```

This is a relatively small sample for the MLE, but we see that the asymptotic
distribution is still a good approximation.
...
Next, we plot the densities of the asymptotic sampling distribution
and the approximation of the asymptotic sampling distribution.
```{r,message=F,warning=F,echo=F,fig.align='center',fig.width=6,fig.height=4}
data.asymp <- rnorm(B,mean=rate,sd=rate/sqrt(n))
data <- data.frame(values=c(data.asymp,data.approx),
    group=c(rep("true asymptotic",B),rep("estimated asymptotic",B)))
