...
# Introduction
To demonstrate the `algebraic.mle` R package, we consider the relatively
simple case of a random sample from the exponentially distributed random
variables.
...
# Generating a sample
We generate a random sample $X_i \sim \operatorname{EXP}(\lambda=1)$ for
$i=1,\ldots,n$ with:
...
Note that the simulation size is only for estimating $f(\lambda)$, but the
and we can make this as large as we wish, but it will only be as good as the
original estimate of $\theta$, which we cannot control.
