...
`algebraic.mle` provides an algebra over maximum likelihood estimators (MLEs).
Under regularity conditions, any MLE is asymptotically normal with variance
given by the inverse Fisher information. The package exploits this structure
with operations for composition (`joint`, `combine`), transformation (`rmap`),
and bridging to distribution algebra (`as_dist`).
...
As you can see, we return an `mle_fit` object, and then we give it a sub-class
`mle_normal` so we can specialize some of the methods for MLE of the normal
distribution, e.g., `bias.mle_normal` which we show later.
...
```{r fig.width=4, fig.height=4, echo=FALSE, fig.align="center", fig.cap="Sampling distribution of the MLEs.", warning=FALSE, cache=TRUE, eval=has_ggplot2}
tmp <- colMeans(mles)
...
FIM, we can combine multiple independent MLEs of the same $\theta$ — each
estimated from a different sample — into a single MLE that incorporates the
Fisher information from all samples.
...
$N=100$ observations from $\mathcal{N}(\theta)$ and compute the MLE
for the observed sample, denoted by $\hat\theta$.

We then divide the observed sample into $r=5$ sub-samples, each of size
$N/r=20$, and compute the MLE for each sub-sample, denoted by
$\theta^{(1)},\ldots,\theta^{(r)}$.
...
First, we show the combined MLE and its variance-covariance:
```{r}
params(mle.wt)
vcov(mle.wt)
```
...
Unfortunately, $\hat\theta$ is a much better estimator of $\theta$ than
...
the combined MLE to perform poorly.
